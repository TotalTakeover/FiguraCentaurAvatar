...
function events.RENDER(delta, context)
	if context == "FIRST_PERSON" or context == "RENDER" or (not client.isHudEnabled() and context ~= "MINECRAFT_GUI") then
		...
		if pose.stand or pose.crouch then
			...
			posOffset  = posOffset - vec(0, (pose.sleep or pose.elytra) and 4 or 14, pose.sleep and -38 or 0) / 16
			nameOffset = posOffset - vec(0, (pose.sleep and -12 or -2) * modelEyeHeight, pose.sleep and 0 or -16) / 16
		...
		local cameraPos = centaurParts.Body:partToWorldMatrix():apply() + vec(0, 0.2, 0) + client:getCameraDir() * 0.1
